...
TEST(HashTable, can_insert_el_1) {
	HashTable res(10);
	P a;
	a.id = 3;
	res.Insert(a);
	EXPECT_EQ(res.Get_el(3).Head()->data.id, 3);
}

TEST(HashTable, can_insert_el_2) {
	HashTable res(10);
	P a,b;
	a.id = 3;
	b.id = 8;
	res.Insert(a);
	res.Insert(b);
	EXPECT_EQ(res.Get_el(3).Head()->next->data.id, 8);
}

TEST(HashTable, can_remove_1) {
	HashTable res(10);
	P a, b;
	a.id = 3;
	b.id = 8;
	...
	res.Insert(b);
	res.Remove(3);
	EXPECT_EQ(res.Get_el(3).Head()->data.id, 8);
}

TEST(HashTable, can_remove_2) {
	HashTable res(10);
	P a, b;
	a.id = 3;
	b.id = 8;
