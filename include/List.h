...
public:
	List(int n) {
		head = new Node<int>; // âûäåëÿåòñÿ ïàìÿòü ïîä îäèí êâàäðàòèê
		size = n;
		Node<int>* tmp = head; // ñîçäàåì êâàäðàòèê êàê õåàä
		for (int i = 0; i < n; i++) {
			Node<int>* tmp2 = new Node<int>; // ñîçäàåì êàêîé-òî êâàäðàòèê
	...
	List<T>() {
		head = new Node<T>; // âûäåëÿåòñÿ ïàìÿòü ïîä îäèí êâàäðàòèê
		size = 1;
	}
	void insert(T data, Node<T>* prev) {
		Node<T>* a = new Node<T>; // ñîçäàëè êâàäðàòèê à
		a->data = data; // ïîëîæèëè òóäà äàííûå
		Node<T>* b = prev->next; // â b ìû ñîõðàíÿåì àäðåñ ñëåäóþùåãî
	...
	int& Getelem(Node<int>* i) { // áåç ññûëêè âåðíåòñÿ êîïèÿ
		return i->data;
	}
	int search(int k) {
		int i = 0;
